...
Windows에서는 프로젝트 폴더에서 아래 스크립트를 권장합니다. 기존 PySide6가 꼬여 있어도 가상환경을 새로 만들고 검증까지 실행합니다.
...
.\.venv\Scripts\python tools\diagnose_qt.py
.\.venv\Scripts\python main.py
```

Linux/macOS에서는 활성화 명령만 환경에 맞게 바꿔 설치하면 됩니다.
...
`DLL load failed while importing QtCore` 또는 `지정된 프로시저를 찾을 수 없습니다`가 나오면:

1. `install_windows.ps1`로 새 가상환경을 다시 만듭니다.
2. 여전히 실패하면 Microsoft Visual C++ Redistributable 2015-2022 x64를 설치합니다.
3. PC에 Anaconda, QGIS, 다른 Qt 앱 경로가 PATH에 섞여 있으면 이 앱은 `.venv`에서 실행하세요.

의존성은 QtCore DLL 호환성을 위해 `PySide6==6.10.3`으로 고정되어 있습니다. 임의로 최신 PySide6로 올리면 일부 Windows/Python 조합에서 QtCore 로드가 실패할 수 있습니다.
